$(function(){
  ...
  $(document).on('change', 'input[type= "file"].upload-image',function(event) {
    ...
    //画像プレビューのコード
    if(count_data >= 4) {
      console.log(count_data) //data-image(画像の数)が５個以上なら画像プレビュー ２段目表示
      dropzone2.css({
        'display': 'block'
      })
      ...
      //   console.log('1')
      //   dropzone2.css({
      //     'width': `calc(100% - (125px * ${count_data - 4}))`  //画像が増える度にクリックエリアが減っていく・・下段
      //   })
      // })

      if(count_data == 8) {  //data-image(画像の数)が９個になれば pタグをカメラアイコンに変更
        ...
        // })
        dropzone.css({
          'width': `calc(100% - (125px * ${count_data + 1}))`  //画像が増える度にクリックエリアが減っていく・・上段
        })
      }
      if(count_data == 1) {
        dropzone.find('p').replaceWith('<i class="fa fa-camera"></i>') //data-image(画像の数)が2個になれば pタグをカメラアイコンに変更
      }
    if(count_data == 9) {  //data-image(画像の数)が10個になれば下段のクリックエリアを非表示
      dropzone2.css({
        'display': 'none'
      })
      return;
    ...
    $('img:last').attr('data-image', `${count_data}`)
  });
  ...
  $(document).on('click', '.delete', function() {
    ...
    if (count_data >= 4) {
      ...
      // })
      dropzone2.css({
        'width': `calc(100% - (125px * ${count_data - 4}))`
      })
      if(images.length == 9) {
        dropzone2.find('p').replaceWith('<i class="fa fa-camera"></i>')
      }
      ...
      // })
      dropzone.css({
        'width': `calc(100% - (125px * ${count_data + 1}))`
      })
    }
    if(count_data == 3) {
      dropzone2.css({
